...
function OutwardDisplay(props) {
  ...
  const handleSubmit = () => {
    ...
    const filter = (data.filter(items => {
      return (items.GATE_ENTRY_NO.toString().toLowerCase() == id.toLowerCase() && (items.TYPE == 4 || items.TYPE == 5))
    })
    )
    // console.log(filter)
    if (filter.length > 0) {
      navigate('/DisplayOutward/Detail', { state: filter })
    }
    else {
        ...
        <div className='poLogin'>
          ...
          <div className='rightdiv'>
            ...
            <div className='rightIcon'>
              <div>
                <h1 className='h2'>Display Outward Entry</h1>
...
export default connect(mapStateToProps, { GetEntry })(OutwardDisplay)
