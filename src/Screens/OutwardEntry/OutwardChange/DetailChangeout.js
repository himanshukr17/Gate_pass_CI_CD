...
function DetailChangeout() {
  ...
  const handleSubmit = () => {
    ...
    // seterror(err)
    if (!hasErr) {
      navigate('/ChangeOutward/ItemFields', { state: { previousdata, data } })
    }
  }
    ...
    <>
      ...
      <div className='headinggg'>
        <div className='headingStyle'>
          ...
          <p className='po'>Change Outward Gate Entry</p>
        </div>

        {/* <Link to="/ChangeOutward/ItemFields" style={{ color: 'black' }}><p className='po' >View Item Feilds</p></Link> */}
      </div>
        ...
        <div className='inner'>
          ...
          <div className='grid-container'>
            <div className='inputFeildDesign'>
              ...
              <input
                ...
                disabled
                value={previousdata[0].INVOICE_NO}
              />
            </div>
          ...
          <div className='grid-container'>
            ...
            <div className='inputFeildDesign'>
              ...
              <input
                type="datetime-local"
                min={new Date(previousdata[0].INWARD_DATETIME).toISOString().split(".")[0]} // Minimum date and time
                max={new Date().toISOString().split(".")[0]} // Maximum date and time
                value={
                  previousdata[0].OUTWARD_DATETIME
                    ? new Date(previousdata[0].OUTWARD_DATETIME).toISOString().split(".")[0] // ISO format for datetime-local
                    : undefined
                }
                disabled={!!previousdata[0].OUTWARD_DATETIME}
                placeholder="Please enter Vehicle Outward Date & Time"
                onChange={(event) => {
                  const selectedDateTime = new Date(event.target.value);
                  const now = new Date();

                  // Validate for future date and time
                  if (selectedDateTime > now) {
                    seterror({ ...error, OUTDATE: "Future date and time are not allowed" });
                    return;
                  }

                  // Update value if valid
                  setValue({ OUTDATE: event.target.value });
                  seterror({ ...error, OUTDATE: '' });
                }}
              />
              {error.OUTDATE && <span className="error">{error.OUTDATE}</span>}
            </div>
...
export default DetailChangeout