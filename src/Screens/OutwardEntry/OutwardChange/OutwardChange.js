...
function OutwardChange(props) {
  ...
  const handleSubmit = () => {
    ...
    const filter = (data.filter(items => {
      return (items.GATE_ENTRY_NO.toString().toLowerCase() == id.toLowerCase() && (items.TYPE == 4 || items.TYPE == 5) && (items?.FLAG != 3))
    })
    )
    // console.log(filter)
    if (filter.length > 0) {
      navigate('/ChangeOutward/Detail', { state: filter })
    }
    else {
        ...
        <div className='poLogin'>
          ...
          <div className='rightdiv'>
            ...
            <div className='rightIcon'>
              <div>
                <h1 className='h2'>Change Outward Entry</h1>
...
export default connect(mapStateToProps, { GetEntry })(OutwardChange)
