...
function InwardChange(props) {
  ...
  const handleSubmit = () => {
    ...
    const filter = (data.filter(items => {
      return (items.GATE_ENTRY_NO.toString().toLowerCase() == id.toLowerCase() && (items.TYPE == 1 || items.TYPE == 2 | items.TYPE == 3) && (items?.FLAG != 3))
    })
  )
  // console.log("filter", filter)
   if (filter[0]?.OUTWARD_INDICATOR ==2){
    toast.error('This Entry has already been exited outward')
   }
    else if (filter.length > 0  && filter[0]?.OUTWARD_INDICATOR == 1) {
      navigate('/ChangeInward/Detail', { state: filter })
    }
    else {
        ...
        <div className='poLogin'>
          ...
          <div className='rightdiv'>
            ...
            <div className='rightIcon'>
              <div>
                <h1 className='h2'>Change Inward Entry</h1>
...
export default connect(mapStateToProps, { GetEntry })(InwardChange)
